...
- (void)handleMethodCall_Do_Direct:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    switch([index intValue]){
        ...
        case starcore_getAssetsPath :
        {
            VS_CHAR path_temp[512];

            NSString *respaths = [[NSBundle mainBundle] resourcePath];
            const VS_CHAR *res_cpath = [respaths UTF8String];

            sprintf(path_temp,"%s/flutter_assets",res_cpath);
            if( vs_file_exist(path_temp)){
                result(respaths);
            }else{
                result([NSString stringWithFormat:@"%@/Frameworks/App.framework",respaths]);
            }
            break;
        }
        case starcore_isAndroid :
        ...
        {
            long FunctionPtr[] = {ENV_MODULECALL};
            int number = (sizeof(FunctionPtr) / sizeof(long));
            if( number == 0 )
                result([NSNumber numberWithLong:(long)0]);
            else{
                int index = rand() % number;
                result([NSNumber numberWithLong:(long)FunctionPtr[index]]);
            }
        }
            break;
        case starcore_init :
...
#if 0
                /*test the path*/
                sprintf(python_home,"%s/flutter_assets/starfiles",res_cpath);
                if( vs_file_exist(python_home)){
                    ...
                    sprintf(python_home,"%s/Frameworks/App.framework/flutter_assets/starfiles/python",res_cpath);
                    sprintf(python_path,"%s/Frameworks/App.framework/flutter_assets/starfiles/%s.zip",res_cpath,ENV_WITHPYTHON);
                }
#endif
                sprintf(python_home,"%s/python",res_cpath);
                sprintf(python_path,"%s/%s.zip",res_cpath,ENV_WITHPYTHON);
                
                if(vs_file_exist(python_path) == VS_FALSE ){
                    sprintf(python_path,"%s/Frameworks/starflut.framework/%s.zip",res_cpath,ENV_WITHPYTHON);
                    if(vs_file_exist(python_path) == VS_FALSE ){
                        NSLog(@"python library [%s] does not found",python_path);
                    }else{
                        sprintf(python_home,"%s/Frameworks/starflut.framework/python",res_cpath);
                        sprintf(python_path,"%s/Frameworks/starflut.framework/%s.zip",res_cpath,ENV_WITHPYTHON);
                    }
                }else{
                    sprintf(python_home,"%s/python",res_cpath);
...
- (id)handleMethodCall_Do:(FlutterMethodCall*)call {
    ...
    switch([index intValue]){
        ...
        case starcore_InitCore:
        {
            NSArray *plist = (NSArray *)call.arguments;
            SRPControlInterface -> SRPLock();
            /*VS_BOOL ShowMenuFlag,VS_BOOL ShowOutWndFlagstarcore_releaseScriptGIL*/
